...
class AMWAELoss(nn.Module):
    ...
    def __init__(
        ...
        use_confidence_weighted_vlm: bool = False,
    ):
        super().__init__()
        ...
        # Plugin I-5: per-pair VLM-confidence weights on the masked bucket
        # (weight = lambda_vlm * confidence) instead of one global lambda_vlm.
        # Requires a (T, K_max) confidence tensor at forward time; without it
        # the loss silently falls back to the global weight (see warning).
        self.use_confidence_weighted_vlm = use_confidence_weighted_vlm
        self._warned_no_confidence = False
    ...
    def forward(
        ...
        vlm_confidence: Optional[torch.Tensor] = None,
        ...
        Args:
            ...
            vlm_confidence: (T, K_max) float in [0, 1] or None — per-pair VLM
                label confidence (plugin I-5, use_confidence_weighted_vlm).
        """
    ...
    def _compute_scene_graph_loss(
        ...
        # Plugin I-5: with use_confidence_weighted_vlm and a confidence tensor,
        # each masked pair instead contributes (λ_vlm * confidence)/N.
        if pair_masked.any():
            ...
            pair_w = None  # (K_m,) per-pair weights, or None → global λ_vlm
            if self.use_confidence_weighted_vlm:
                if vlm_confidence is not None:
                    pair_w = (
                        self.lambda_vlm
                        * vlm_confidence.to(device)[valid][pair_masked].clamp(0.0, 1.0)
                    )
                elif not self._warned_no_confidence:
                    logger.warning(
                        "[AMWAELoss] use_confidence_weighted_vlm=True but no "
                        "vlm_confidence tensor supplied — falling back to the "
                        "global lambda_vlm. Regenerate features with per-label "
                        "VLM confidences to activate this plugin."
                    )
                    self._warned_no_confidence = True
            ...
            if self._label_smoother is not None:
                ...
                att_elem = F.cross_entropy(m_att_pred, m_att_gt, reduction="none")

            spa_elem = F.binary_cross_entropy_with_logits(
                m_spa_pred, m_spa_gt, reduction="none",
            ).sum(dim=-1)  # (K_m,)
            con_elem = F.binary_cross_entropy_with_logits(
                m_con_pred, m_con_gt, reduction="none",
            ).sum(dim=-1)  # (K_m,)

            if pair_w is not None:
                losses["masked_att"] = (att_elem * pair_w).sum() / N
                losses["masked_spa"] = (spa_elem * pair_w).sum() / N
                losses["masked_con"] = (con_elem * pair_w).sum() / N
            else:
                losses["masked_att"] = att_elem.sum() * self.lambda_vlm / N
                losses["masked_spa"] = spa_elem.sum() * self.lambda_vlm / N
                losses["masked_con"] = con_elem.sum() * self.lambda_vlm / N
        else:
