...
class ScaffoldTokenizer(nn.Module):
    ...
    Args:
        ...
        use_ema_target: Reconstruction targets come from an EMA copy of the
            visual projector (data2vec-style) instead of the live projector.
            The live projector's own detached output is a *moving* target the
            model can chase toward easy-to-reconstruct collapse; the EMA copy
            keeps the target slow and stable. Default True (post-fix I-0).
        ema_momentum: EMA decay for the target projector update.
    """
    ...
    def __init__(
        ...
        # EMA target projector — never trained by gradient, follows the live
        # projector by momentum update; provides the reconstruction target.
        if self.use_ema_target:
            import copy
            self.target_projector = copy.deepcopy(self.visual_projector)
    ...
    def forward(
        ...
        # Reconstruction target: EMA projection (stable) or, legacy, the live
        # projection detached (moving target — kept only for A/B runs).
        if self.use_ema_target:
            if self.training:
                self._update_ema_target()
            with torch.no_grad():
                original_visual = self.target_projector(visual_features)
        else:
